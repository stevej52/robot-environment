...
if [ "$DO_UNITS" -eq 1 ]; then
    ...
    say "installing isaac-vo, jetnano-robot, jetnano-slam and wifi-watchdog services"
    run sudo cp "$UNITS"/isaac-vo.service "$UNITS"/jetnano-robot.service "$UNITS"/jetnano-slam.service "$UNITS"/wifi-watchdog.service /etc/systemd/system/
    run sudo systemctl daemon-reload
    run sudo systemctl enable isaac-vo.service jetnano-robot.service jetnano-slam.service wifi-watchdog.service
fi
